...
class AdminFeatureController extends Controller
{
    ...
    public function payments(Request $request): Response
    {
        ...
        return $this->renderFeature('Keuangan & Bukti Bayar', 'Verifikasi bukti pembayaran dan lihat status iuran dari satu halaman.', [
        ...
        ], ['ID & Tanggal', 'Member', 'Tipe & Keterangan', 'Nominal', 'Metode', 'Bukti'], 'Tidak ada data pembayaran pending.', 'payments', $payments->map(fn (Payment $payment) => [
    ...
    public function monthlyDues(Request $request): Response
    {
        ...
        return $this->renderFeature('Input Iuran', 'Input dan pantau iuran bulanan member langsung dari halaman ini.', [
            ...
            ['label' => 'Total Masuk', 'value' => 'Rp '.number_format((float) $payments->sum('paid_amount'), 0, ',', '.'), 'tone' => 'info'],
        ], ['Nama Member', 'No. HP', 'Tipe Iuran', 'Nominal', 'Tanggal Bayar', 'No. Transaksi', 'Status'], 'Belum ada data iuran bulan ini.', 'monthly-dues', $athletes->map(function (Athlete $athlete) use ($setting, $payments, $paidAthleteIds, $month, $year): array {
            ...
            return [
                ...
                'Status' => $paidAthleteIds->contains($athlete->athlete_id) ? 'Lunas' : 'Belum bayar',
            ];
        })->values()->all(), [
            'billingSettings' => [
    ...
    public function members(Request $request): Response
    {
        ...
        return $this->renderFeature('Manajemen Anggota', 'Total '.$athletes->count().' anggota terdaftar', [], ['No.', 'Anggota', 'Pribadi', 'Sabuk', 'Kontak', 'Dokumen', 'Status'], 'Tidak ada data anggota', 'members', $athletes->values()->map(fn (Athlete $athlete, int $index) => [
            'No.' => (string) ($index + 1),
    ...
    public function events(Request $request): Response
    {
        ...
        return $this->renderFeature('Event / UKT', 'Kelola event, UKT, riwayat, dan pendaftar dari satu halaman.', [
            ['label' => 'Total Event', 'value' => (string) $events->count(), 'tone' => 'info'],
            ['label' => 'Event Aktif', 'value' => (string) $events->whereIn('status', ['ACTIVE', 'OPEN', 'PUBLISHED'])->count(), 'tone' => 'success'],
            ['label' => 'Riwayat', 'value' => (string) $events->where('e_date', '<', now())->count(), 'tone' => 'warning'],
        ], ['Event', 'Tanggal & Waktu', 'Pendaftar', 'Biaya', 'Tipe & Status'], 'Tidak ada data event', 'events', $events->map(fn (Event $event) => [
            'Event' => $event->e_name,
            ...
            'Tipe & Status' => ($event->level ?? '-').' · '.($event->status ?? '-'),
        ])->values()->all());
    }
    ...
    public function locations(Request $request): Response
    {
        ...
        ], ['Nama Lokasi & Kelas', 'Alamat', 'Status'], 'Tidak ada data lokasi', 'locations', [], [
    ...
    public function weeklySchedules(Request $request): Response
    {
        ...
        return Inertia::render('AdminFeaturePage', [
            ...
            'subtitle' => 'Jadwal latihan rutin dan sesi minggu ini dalam satu halaman.',
            'metrics' => [
    ...
    {
        return $this->weeklySchedules($request);
    }
    ...
    private function beltOptions()
    {
        ...
        return $groupBelts
            ...
            ->unique()
            ->values()
            ->map(fn (string $belt) => ['value' => $belt, 'label' => $belt]);
    }
