...
class FreshApplicationSeeder extends Seeder
{
    ...
    public function run(): void
    {
        DB::transaction(function (): void {
            ...
            [$parents, $coaches] = $this->seedRoleProfiles($users);
            $branches = $this->seedBranches();
            $categories = $this->seedTrainingGroups();
            $classes = $this->seedClasses($branches, $categories, $coaches);
            $athletes = $this->seedAthletes($users, $parents, $branches, $categories, $classes);
            ...
            $this->seedSessions($branches, $classes, $coaches);
    ...
    private function seedUsers(): array
    {
        $definitions = [
            ...
            'multi' => ['Dewi Multi Peran', 'multirole@rfis.test', ['coach', 'parent', 'athlete'], 'FEMALE', '1998-08-08'],
            'multiChild' => ['Raka Multi', 'child@rfis.test', ['athlete'], 'MALE', '2014-04-12'],
        ];
        $users = [];

        foreach ($definitions as $index => [$name, $email, $roles, $gender, $birthday]) {
            $user = User::query()->updateOrCreate(['email' => $email], [
                ...
                'role' => $roles[0],
                'bday' => $birthday,
                'phone' => '0800000000'.str_pad((string) (count($users) + 1), 2, '0', STR_PAD_LEFT),
    ...
    {
        return [
            'junior' => TrainingGroup::query()->updateOrCreate(['name' => 'Junior'], [
                'description' => 'Junior athlete category.', 'is_active' => true,
            ]),
            'competition' => TrainingGroup::query()->updateOrCreate(['name' => 'Prestasi'], [
                'description' => 'Competition athlete category.', 'is_active' => true,
            ]),
            'beginner' => TrainingGroup::query()->updateOrCreate(['name' => 'Pemula'], [
                'description' => 'Beginner athlete category.', 'is_active' => true,
            ]),
        ];
    }

    private function seedClasses(array $branches, array $categories, array $coaches): array
    {
        $today = now()->isoWeekday();
        $classes = [
            'junior' => Group::query()->updateOrCreate(['group_name' => 'Junior Sparring'], [
                'branch_id' => $branches['central']->branch_id,
                'training_group_id' => $categories['junior']->id,
            ...
            'competition' => Group::query()->updateOrCreate(['group_name' => 'Competition Team'], [
                ...
                'training_group_id' => $categories['competition']->id,
            ...
            'private' => Group::query()->updateOrCreate(['group_name' => 'Private Performance'], [
                ...
                'end_time' => '19:00',
                'min_belt' => null,
                'description' => 'Private class with multiple athletes and coaches.',
                'is_active' => true,
            ]),
        ];
    ...
    private function seedAthletes(array $users, array $parents, array $branches, array $categories, array $classes): array
    {
        $definitions = [
            'adit' => [$users['athlete'], $classes['junior'], $categories['junior'], $parents['parent'], $branches['central'], 150.5, 42.3, 'GEUP_8', '2024-07-01'],
            'nadia' => [$users['nadia'], $classes['junior'], $categories['junior'], $parents['parent'], $branches['central'], 147.0, 39.8, 'GEUP_9', '2024-07-01'],
            'rafi' => [$users['rafi'], $classes['competition'], $categories['competition'], null, $branches['east'], 154.2, 45.1, 'GEUP_5', '2025-01-10'],
            'putri' => [$users['putri'], $classes['competition'], $categories['competition'], null, $branches['east'], 156.3, 47.4, 'GEUP_6', '2025-01-10'],
            'multi' => [$users['multi'], $classes['competition'], $categories['competition'], null, $branches['east'], 165.0, 55.0, 'DAN', '2022-01-15'],
            'multiChild' => [$users['multiChild'], $classes['junior'], $categories['junior'], $parents['multi'], $branches['central'], 140.0, 35.0, 'GEUP_10', '2026-07-23'],
        ];
        $athletes = [];

        foreach ($definitions as $key => [$user, $class, $category, $parent, $branch, $height, $weight, $belt, $joinedAt]) {
    ...
    private function seedSessions(array $branches, array $classes, array $coaches): void
    {
        ...
        $session = TrainingSession::query()->updateOrCreate(
            ...
            [
                ...
                'dedicated_athlete_id' => $class->dedicated_athlete_id,
                'title' => $class->group_name,
                'location' => $class->branch?->location,
                'start_time' => $class->start_time,
                'end_time' => $class->end_time,
                'status' => 'CONFIRMED',
            ...
            foreach ($session->attendances as $attendanceIndex => $attendance) {
                ...
                $attendance->update([
                    ...
                    'checked_in_at' => $present
                        ? Carbon::parse((string) $session->session_date)
                            ->setTimeFromTimeString(substr((string) $session->start_time, 0, 8))
                            ->addMinutes(4)
                        : null,
    ...
    {
        $reference = 900000000000000;
        $nextReference = function () use (&$reference): int {
            return $reference++;
        };
        $parentByAthlete = [
        ...
        foreach ($athletes as $key => $athlete) {
            $completed = Payment::query()->create([
                ...
                'notes' => 'Seeded completed tuition.',
            ]);
    ...
    private function seedChampionships(array $users, array $athletes): void
    {
        ...
        $resultRegistration = EventRegistration::query()->create([
            'athlete_id' => $athletes['adit']->athlete_id,
        ...
        UserAchievement::query()->create([
            ...
            'event_id' => $completed->event_id,
            'event_registration_id' => $resultRegistration->evrid,
        ...
        $pending = EventRegistration::query()->create([
    ...
    private function seedProfileRecords(array $users): void
    {
        ...
        UserAchievement::query()->create([
            'user_id' => $users['multi']->id,
            'championship_name' => 'Kejuaraan Internal RFIS',
            'medal' => 'GOLD',
            ...
            'notes' => 'Manual achievement for the multi-role demo.',
        ]);
    }
}
