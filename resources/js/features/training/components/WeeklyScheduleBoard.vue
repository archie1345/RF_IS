...
const todayLabel = computed(() => today.toLocaleDateString('en-GB', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }).toUpperCase());
...
function typeTone(schedule: WeeklyScheduleCard): string {
    ...
    if (value.includes('prestasi') || value.includes('advanced') || value.includes('senior')) return 'border-red-200 bg-red-50 text-red-700';
    ...
    <section class="rounded-2xl bg-slate-50 p-4 text-slate-950 shadow-sm ring-1 ring-slate-200 md:p-6">
        <div class="mb-7 grid gap-4 xl:grid-cols-[1fr_auto_1fr] xl:items-start">
            <div>
                ...
                <p class="mt-1 text-xs font-black uppercase text-red-500">DOJANG: {{ branchLabel }}</p>
                <p class="mt-1 text-sm font-medium text-slate-500">{{ subtitle }}</p>
            </div>

            <div class="justify-self-start rounded-full border border-slate-300 bg-white px-5 py-2 text-sm font-black shadow-sm xl:justify-self-center">
                <span class="mr-2 inline-flex size-2 rounded-full bg-blue-500"></span>
                TODAY: {{ todayLabel }}
            </div>

            <div class="flex justify-start gap-3 xl:justify-end">
                <button type="button" class="inline-flex size-12 items-center justify-center rounded-xl bg-red-100 text-red-600 transition hover:bg-red-200" @click="emit('refresh')">
                    <RefreshCcw class="size-6" />
                </button>
                <div v-if="showManagementHint" class="flex min-h-12 items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 text-sm font-black uppercase text-slate-500 shadow-sm">
        ...
        <div class="grid gap-3 md:grid-cols-7">
            <div
                ...
                :class="day.id === todayDay ? 'border-red-500 bg-red-500 text-white' : 'border-slate-200 text-slate-950'"
            >
                <p class="text-xl font-black leading-none">{{ day.name }}</p>
                <p class="mt-1 text-xs" :class="day.id === todayDay ? 'text-white' : 'text-slate-400'">{{ day.english }}</p>
            </div>
        </div>

        <div class="mt-4 grid gap-3 md:grid-cols-7">
            <div v-for="day in days" :key="`body-${day.id}`" class="min-h-[280px] rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <template v-if="(schedulesByDay.get(day.id) ?? []).length">
                    ...
                    >
                        <div class="mb-3 border-b border-slate-100 pb-3">
                            <h3 class="text-base font-black">{{ schedule.group && schedule.group !== 'All groups' ? schedule.group : schedule.title }}</h3>
                        ...
                        <div class="space-y-4 text-xs font-black uppercase text-slate-500">
                            <div class="flex gap-3">
                                <UserRound class="mt-0.5 size-4 shrink-0 text-red-500" />
                                <div><p>Instruktur</p><p class="text-sm normal-case text-slate-950">{{ schedule.coach || 'TBA' }}</p></div>
                            </div>
                            <div class="flex gap-3">
                                <Clock3 class="mt-0.5 size-4 shrink-0 text-blue-500" />
                                <div><p>Waktu</p><p class="text-sm text-slate-950">{{ schedule.start_time || '--:--' }} - {{ schedule.end_time || '--:--' }}</p></div>
                            </div>
                            <div class="flex gap-3">
                                <UsersRound class="mt-0.5 size-4 shrink-0 text-orange-500" />
                                <div><p>Peserta</p><p class="text-sm text-slate-950">{{ schedule.athletes_count ?? '-' }}</p></div>
                            </div>
                            <div class="flex gap-3">
                                <MapPin class="mt-0.5 size-4 shrink-0 text-emerald-500" />
                                <div><p>Lokasi</p><p class="text-sm normal-case text-slate-950">{{ schedule.location || schedule.branch || 'RTFCM' }}</p></div>
                            </div>
                        </div>

                        <div v-if="canManage && schedule.can_manage" class="mt-4 flex gap-2 border-t border-slate-100 pt-3">
                            <button type="button" class="rounded-lg border px-3 py-1 text-xs font-bold hover:bg-slate-50" @click="emit('edit', schedule)">Edit</button>
                            <button type="button" class="rounded-lg border px-3 py-1 text-xs font-bold text-red-600 hover:bg-red-50" @click="emit('delete', schedule)">Delete</button>
                        </div>
                    </article>
                </template>
                <div v-else class="flex h-full min-h-[240px] flex-col items-center justify-center text-xs font-black uppercase text-slate-300">
        ...
        <div class="mx-auto mt-10 flex w-fit flex-wrap items-center justify-center gap-5 rounded-full border border-slate-200 bg-white px-8 py-4 text-sm font-bold text-slate-500 shadow-lg">
            <div class="flex items-center gap-2"><Info class="size-4 text-blue-500" /> Jadwal sinkron otomatis dengan Manajemen Kelas</div>
            <div class="hidden h-6 w-px bg-slate-200 md:block"></div>
            <div v-for="item in legendItems" :key="item.label" class="rounded-full border px-3 py-1 text-xs font-black text-slate-950">
