    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex min-w-0 flex-1 flex-col gap-5 p-3 sm:p-4 md:p-6">
            ...
            >
                <template #actions>
                    ...
                    <Button v-if="props.canManageCoaches" type="button" variant="outline" @click="coachModalOpen = true">
                        Tambah pelatih
                    </Button>
                ...
                <dl class="grid gap-3 rounded-xl border bg-card p-4 text-sm sm:grid-cols-2 xl:grid-cols-5">
                    <div><dt class="text-xs text-muted-foreground">Tanggal</dt><dd class="mt-1 font-semibold">{{ props.event.date }}</dd></div>
                    <div><dt class="text-xs text-muted-foreground">Batas pendaftaran</dt><dd class="mt-1 font-semibold">{{ props.event.registration_deadline }}</dd></div>
                    <div><dt class="text-xs text-muted-foreground">Lokasi</dt><dd class="mt-1 font-semibold">{{ props.event.location }}</dd></div>
                    <div><dt class="text-xs text-muted-foreground">Biaya masuk</dt><dd class="mt-1 font-semibold">Rp {{ props.event.entry_fee.toLocaleString('id-ID') }}</dd></div>
                    <div><dt class="text-xs text-muted-foreground">Status</dt><dd class="mt-1 font-semibold">{{ props.event.status }}</dd></div>
                    <div v-if="props.event.gmaps_url" class="sm:col-span-2 xl:col-span-5">
                        <a :href="props.event.gmaps_url" target="_blank" rel="noreferrer" class="font-medium text-primary hover:underline">Buka lokasi di peta</a>
                    </div>
                </dl>
            </PageSection>
            ...
            >
                <template #row-actions="{ row }">
                    <ActionButtonsRow>
                        <Button v-if="row.can_edit_registration === true" type="button" size="sm" variant="outline" @click="openRegistrationEdit(row)">
                            {{ row.is_own_registration === true ? 'Ubah pendaftaran' : 'Ubah entri' }}
                        </Button>
                        <Button v-if="props.isAdmin && athleteProfileUrl(row)" as-child type="button" size="sm" variant="outline">
                            <Link :href="athleteProfileUrl(row) ?? '#'">Profil</Link>
                        </Button>
                        <Button v-if="props.canRecordResult" type="button" size="sm" variant="outline" @click="openResultForm(row)">
                            Catat hasil
                        </Button>
                        <Button v-if="props.canDeleteRegistration" type="button" size="sm" variant="destructive" @click="removeRegistration(row)">
                            Hapus
                        </Button>
                    </ActionButtonsRow>
                </template>
        ...
        <FormModal :open="participantModalOpen" max-width-class="max-w-2xl" @close="participantModalOpen = false">
            <PageSection
                ...
                :description="props.isAdmin && !props.event.registration_open
            ...
            >
                <form class="grid gap-4" @submit.prevent="submitParticipant">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <FormSelectField id="managed-event-category" v-model="participantForm.category" label="Kategori" :options="categoryOptions" required :error="participantForm.errors.category" />
                        <FormInputField id="managed-event-classification" v-model="participantForm.classification" label="Klasifikasi" :error="participantForm.errors.classification" />
                        <FormInputField id="managed-event-class" v-model="participantForm.class_name" label="Kelas" :error="participantForm.errors.class_name" />
                        <FormInputField id="managed-event-division" v-model="participantForm.division" label="Divisi" :error="participantForm.errors.division" />
                    </div>
                    <FormInputField id="managed-event-team" v-model="participantForm.team_contingent" label="Tim/Kontingen" :error="participantForm.errors.team_contingent" />
                    <label class="flex items-start gap-3 rounded-lg border p-3 text-sm">
                        ...
                        <span><strong class="block">Buat tagihan biaya masuk</strong><span class="text-muted-foreground">Matikan untuk pencatatan peserta lama atau event pasca-pelaksanaan.</span></span>
                    </label>
        ...
        <FormModal :open="coachModalOpen && props.canManageCoaches" max-width-class="max-w-xl" @close="coachModalOpen = false">
            <PageSection title="Tambah pelatih" description="Tugaskan pelatih untuk mendampingi kejuaraan ini.">
                <form class="grid gap-4" @submit.prevent="addCoach">
                    <FormSelectField v-if="props.isAdmin" id="championship-coach" v-model="coachForm.coach_id" label="Pelatih" :options="props.coachOptions" placeholder="Pilih pelatih" :error="coachForm.errors.coach_id" />
                    <FormInputField id="championship-coach-role" v-model="coachForm.role" label="Peran" placeholder="Pelatih utama / Pendamping" :error="coachForm.errors.role" />
                    <div class="flex justify-end gap-2"><Button type="button" variant="outline" @click="coachModalOpen = false">Batal</Button><Button type="submit" :disabled="coachForm.processing">Tambahkan</Button></div>
                </form>
            </PageSection>
        </FormModal>
        ...
        <FormModal :open="editModalOpen" max-width-class="max-w-xl" @close="editModalOpen = false">
            <PageSection
                ...
                :description="editingOwnRegistration ? `Dapat diperbarui sampai ${props.event.registration_deadline}.` : 'Perbarui data pertandingan peserta.'"
            >
                <form class="grid gap-4" @submit.prevent="saveRegistrationEdit">
                    <FormSelectField id="edit-category" v-model="editForm.category" label="Kategori" :options="categoryOptions" :error="editForm.errors.category" />
                    <FormInputField id="edit-classification" v-model="editForm.classification" label="Klasifikasi" :error="editForm.errors.classification" />
                    <FormInputField id="edit-class-name" v-model="editForm.class_name" label="Kelas" :error="editForm.errors.class_name" />
                    <FormInputField id="edit-division" v-model="editForm.division" label="Divisi" :error="editForm.errors.division" />
                    <FormInputField id="edit-team" v-model="editForm.team_contingent" label="Tim/Kontingen" :error="editForm.errors.team_contingent" />
                    <p v-if="editForm.errors.registration" class="text-sm text-destructive">{{ editForm.errors.registration }}</p>
                    <div class="flex justify-end gap-2"><Button type="button" variant="outline" @click="editModalOpen = false">Batal</Button><Button type="submit" :disabled="editForm.processing">Simpan</Button></div>
                </form>
            </PageSection>
        </FormModal>

        <FormModal :open="resultModalOpen && props.canRecordResult" max-width-class="max-w-xl" @close="resultModalOpen = false">
            <PageSection title="Catat hasil pertandingan" description="Hasil ini membuat atau memperbarui prestasi atlet.">
                <form class="grid gap-4" @submit.prevent="saveResult">
                    <FormSelectField id="result-medal" v-model="resultForm.medal" label="Medali" :options="medalOptions" :error="resultForm.errors.medal" />
                    <FormInputField id="result-class" v-model="resultForm.class_name" label="Kelas" :error="resultForm.errors.class_name" />
                    <FormInputField id="result-division" v-model="resultForm.division" label="Divisi" :error="resultForm.errors.division" />
                    <FormInputField id="result-category" v-model="resultForm.category" label="Kategori" :error="resultForm.errors.category" />
                    <div class="flex justify-end gap-2"><Button type="button" variant="outline" @click="resultModalOpen = false">Batal</Button><Button type="submit" :disabled="resultForm.processing">Simpan hasil</Button></div>
                </form>
            </PageSection>
        </FormModal>
    </AppLayout>
