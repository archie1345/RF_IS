...
const beltChartStyle = computed(() => {
    ...
    let cursor = 0;
    const segments = beltRows.value.map((row) => {
        const start = cursor;
        cursor += (row.count / totalBeltRows.value) * 360;
...
function isToday(day: number) {
    ...
    return (
        today.getFullYear() === currentYear.value && today.getMonth() === currentMonth.value && today.getDate() === day
    );
}
...
function updateClock() {
    timeString.value = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
}
onMounted(() => {
    updateClock();
        ...
        <div v-if="props.role === 'admin'" class="grid gap-6 xl:grid-cols-2">
            ...
            <section class="rounded-xl border bg-card p-6 shadow-sm">
                ...
                <div class="flex min-h-72 flex-col items-center justify-center gap-6">
                    <div class="relative size-44 rounded-full shadow-inner" :style="beltChartStyle">
                        <div class="absolute inset-10 rounded-full bg-card"></div>
                        <div class="absolute inset-0 flex items-center justify-center">
                            <span class="rounded-full bg-card px-3 py-1 text-xs font-black shadow-sm">{{
                                totalBeltRows
                            }}</span>
                        </div>
                    </div>
                    <div v-if="beltRows.length" class="flex flex-wrap justify-center gap-4 text-xs">
                        <div v-for="row in beltRows" :key="row.label" class="flex items-center gap-2">
                            <span
                                class="size-3 rounded-sm border border-border"
            ...
            <section class="rounded-xl border bg-card p-6 shadow-sm">
                ...
                <div class="grid min-h-48 gap-6 md:grid-cols-[12rem_1fr] md:items-center">
                    <div class="relative mx-auto size-32 rounded-full bg-[conic-gradient(#ef4444_0_100%)]">
                        ...
                        <WalletCards
                            class="absolute top-1/2 left-1/2 size-8 -translate-x-1/2 -translate-y-1/2 text-muted-foreground"
                        />
                    </div>
                    <div class="space-y-4 text-sm">
                        <div class="flex items-center justify-between gap-4">
                            <span class="flex items-center gap-2"
                                ><span class="size-3 rounded-full bg-brand-lime"></span> Sudah Bayar ({{
                                    adminPaymentSummary.paid
                                }})</span
                            ><strong>{{ 100 - adminPaymentSummary.unpaidPercent }}%</strong>
                        </div>
                        <div class="flex items-center justify-between gap-4">
                            <span class="flex items-center gap-2"
                                ><span class="size-3 rounded-full bg-brand-coral"></span> Belum Bayar ({{
                                    adminPaymentSummary.unpaid
                                }})</span
        ...
        <div v-if="props.role === 'athlete'" class="grid grid-cols-1 gap-6 lg:grid-cols-5">
            <div class="rounded-xl border bg-card p-6 shadow-sm lg:col-span-3">
                <div class="mb-4 flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="text-xs text-muted-foreground">
                            Training days are highlighted. Attendance status appears after QR check-in.
                        </p>
                    </div>
                    <span
                        class="rounded-full bg-brand-blue/10 px-3 py-1 text-[10px] font-black tracking-wide text-brand-blue uppercase dark:bg-brand-blue/15 dark:text-brand-blue/80"
                        >Schedule</span
                    >
                </div>
                <div class="mx-auto w-full max-w-sm rounded-xl border bg-background p-4 shadow-sm">
                    <div class="mb-4 flex items-center justify-between">
                        ...
                        <span
                            class="rounded bg-secondary px-2 py-0.5 text-[10px] font-medium text-muted-foreground uppercase"
                            >Latihan</span
                        >
                    </div>
                    ...
                    <div class="grid grid-cols-7 gap-1 text-center text-xs">
                        ...
                        <button
                            ...
                            :class="{
                                ...
                                'bg-brand-blue text-white shadow-sm':
                                    getDayTraining(day).length && !getDayAttendanceStatus(day),
                                'bg-brand-lime text-white shadow-sm': getDayAttendanceStatus(day) === 'PRESENT',
                                ...
                                'bg-muted text-foreground':
                                    isToday(day) && !getDayTraining(day).length && !getDayAttendanceStatus(day),
                        ...
                        >
                            {{ day
                                ...
                                v-if="getDayTraining(day).length"
                                class="absolute bottom-1 h-1 w-1 rounded-full bg-current opacity-80"
                            ></span>
                        </button>
                    </div>
                    <div
                        class="mt-4 flex flex-wrap items-center justify-center gap-3 border-t pt-3 text-[10px] font-semibold text-muted-foreground"
                    >
                        <div class="flex items-center gap-1">
                            <span class="h-2 w-2 rounded-full bg-brand-blue"></span> Training
                        </div>
                        <div class="flex items-center gap-1">
                            <span class="h-2 w-2 rounded-full bg-brand-lime"></span> Present
                        </div>
                        <div class="flex items-center gap-1">
                            <span class="h-2 w-2 rounded-full bg-amber-500"></span> Excused
                        </div>
                        <div class="flex items-center gap-1">
                            <span class="h-2 w-2 rounded-full bg-destructive"></span> Absent
                        </div>
                    </div>
                    <div v-if="selectedDay" class="mt-4 rounded-2xl border bg-muted/40 p-3 text-xs">
                        ...
                        <div v-if="selectedTrainingSessions.length" class="mt-2 grid gap-2">
                            <div
                                v-for="session in selectedTrainingSessions"
                                :key="session.id"
                                class="rounded-xl bg-background p-2"
                            >
                                <p class="font-semibold">{{ session.title }}</p>
                                <p class="text-muted-foreground">
                                    {{ session.time }} · {{ session.branch }} · {{ session.group }}
                                </p>
                            </div>
                        </div>
            ...
            <div class="flex flex-col justify-between rounded-xl border bg-card p-6 shadow-sm lg:col-span-2">
                <div>
                    ...
                    <div class="flex justify-center py-5 text-4xl font-bold text-brand-blue dark:text-foreground">
                        <h1>{{ timeString }}</h1>
                    </div>
                    ...
                    <div class="grid gap-3 rounded-3xl border bg-muted/40 p-4 text-sm">
                        <div class="flex items-start gap-3">
                            <QrCode class="mt-0.5 size-5 text-brand-blue" />
                            <div>
                                <p class="font-bold">1. Open QR scan menu</p>
                                <p class="text-xs text-muted-foreground">Use a phone while logged in as athlete.</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-3">
                            <QrCode class="mt-0.5 size-5 text-brand-blue" />
                            <div>
                                <p class="font-bold">2. Scan coach QR</p>
                                <p class="text-xs text-muted-foreground">The QR opens the secure attendance page.</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-3">
                            <CheckCircle2 class="mt-0.5 size-5 text-brand-lime" />
                            <div>
                                <p class="font-bold">3. Saved and done</p>
                                <p class="text-xs text-muted-foreground">
                                    Athletes do not get manual Attend / Not attend buttons.
                                </p>
                            </div>
                        </div>
                    </div>
